...
int pertence_Area(ponto pos, lista *area){
	no *aux;
	int i, j, maxY, minY, maxX, minX;
	aux = area->inicio;
	//processo de obtencao de maximos e minimos
	minX = aux->p.x;
	minY = aux->p.y;
	aux = aux->prox;
	maxX = aux->p.x;
...
int main(void)
{
	...
	int x1, y1, x2, y2, n,i = 0, x, y, j, k, menor = 100, cont = 0, distT;
	...
	if(pertence_Area(pi, &area[0]) == 1){
		printf("O ponto inicial pertence a area 1\n");
	} else if(pertence_Area(pi, &area[0]) == 0) {
		aux = area[0].inicio;
		for(k = 0; k < 4; k++){ 
        	distT = calculaDist(pi, aux->p);
        	if(distT < menor){
        		menor = distT; 	
			}
			aux = aux->prox;
   ...
   if(pertence_Area(pf, &area[n-1]) == 1){
		printf("O ponto final pertence a area %d\n", n);
	} else if(pertence_Area(pi, &area[0]) == 0) {
		aux = area[n-1].inicio;
		menor = 100;
		for(k = 0; k < 4; k++){ 
        	distT = calculaDist(pf, aux->p);
        	if(distT < menor){
        		menor = distT; 	
			}
			aux = aux->prox;
	...
	printf("\nPara o robo ir de (%.0f,%.0f) ate (%.0f,%.0f), ele passa por %d trechos descobertas\n", pi.x,pi.y, pf.x, pf.y,cont);
}
